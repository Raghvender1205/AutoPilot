...
import os
model = load_model('AutoPilot_1.h5')
...
img = cv2.imread('steering_wheel.jpeg', 0)
print(img.shape)
rows, cols = img.shape
smoothed_angle = 0
...
while (cap.isOpened()):
    ret, frame = cap.read()
    gray = cv2.resize((cv2.cvtColor(frame, cv2.COLOR_RGB2HSV))[
                      :, :, 1], (100, 100))
    steering_angle = keras_predict(model, gray)
    print(steering_angle)
    cv2.imshow('frame', cv2.resize(
        frame, (600, 400), interpolation=cv2.INTER_AREA))
